...
class Db {
    ...
    static public function getInstance($config=array()) {
        ...
        if(!isset(self::$instance[$md5])) {
            ...
            $class  =   $options['lite']?  'lib\db\Lite' :   'lib\\db\\Driver\\'.ucwords($options['type']);
    ...
    static private function parseConfig($config){
        if(!empty($config)){
            ...
            $config = array (
                'type'          =>  SystemFun::Config('DB_TYPE'),
                'username'      =>  SystemFun::Config('DB_USER'),
                'password'      =>  SystemFun::Config('DB_PWD'),
                'hostname'      =>  SystemFun::Config('DB_HOST'),
                'hostport'      =>  SystemFun::Config('DB_PORT'),
                'database'      =>  SystemFun::Config('DB_NAME'),
                'dsn'           =>  SystemFun::Config('DB_DSN'),
                'params'        =>  SystemFun::Config('DB_PARAMS'),
                'charset'       =>  SystemFun::Config('DB_CHARSET'),
                'deploy'        =>  SystemFun::Config('DB_DEPLOY_TYPE'),
                'rw_separate'   =>  SystemFun::Config('DB_RW_SEPARATE'),
                'master_num'    =>  SystemFun::Config('DB_MASTER_NUM'),
                'slave_no'      =>  SystemFun::Config('DB_SLAVE_NO'),
                'debug'         =>  true,
                'lite'          =>  SystemFun::Config('DB_LITE'),
            );
        }
        return $config;
    }
