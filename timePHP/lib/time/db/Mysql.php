...
class Mysql{
    ...
    public function find(){
        ...
        return mysqli_fetch_assoc($this->_query($this->sql));
    }
    //修改
    ...
    public function select(){
        $this->sql="SELECT ".$this->field." FROM ".$this->talbe." ".($this->where=="WHERE "?"":$this->where);
        return $this->get_result_array($this->sql);
    }
    //获取 查询条数
    public function count(){
        $this->sql="SELECT COUNT(*) FROM ".$this->talbe." ".($this->where=="WHERE "?"":$this->where);
        return mysqli_num_rows($this->_query($this->sql));
    }
    //添加
