...
final class Locator
{
    ...
    public function checknamespace($class){
        ...
        if (isset($this->namespaces[$tok])) {
            ...
            $fname=APP_ROOT.$class_;
            if (file_exists($fname . EXT)){
                require_once $fname . EXT;
            }else{
                require_once APP_PATH.DS.$class_. EXT;
            }
            if (self::exists($class, false)) {
                return true;
            }
    ...
    public static function loadTask(){
        foreach(glob(TASK_PATH.'/*/*Task'.EXT) as $task_file){
            $regex='/.*?app(.*?)\.php.*?/';
            if(preg_match_all($regex, $task_file, $matches)){
                $class_name='/app'.$matches[1][0];
            }
            if($class_name==null){
                continue;
            }
            ...
            self::$_taskList[$class_name]["taskObj"]= new $class_name();
            self::$_taskList[$class_name]["taskObj"]->setConfig();
            if (self::$_taskList[$class_name]["taskObj"]->getConfig()["TASK"]["status"]==-1){
                unset(self::$_taskList[$class_name]);
            }
        }
    }
    //加载系统配置
    public static function loadSysConfig(){
        include COMMON_PATH."/SystemFun".EXT;//系统配置文件
        /* $config=include_once COMMON_PATH."/config".EXT;//系统配置文件
        foreach (@$config["TASK"] as $v){
            self::$_taskList[$v["name"]]=[
                "taskObj"=>new SysTask(),
                "configAll"=>$config
            ];
            self::$_taskList[$v["name"]]["taskObj"]->setConfig(["TASK"=>$config["TASK"][$v["name"]]]);
            //加载系统函数
        } */
    }
}