...
class Model{
    ...
    public function __construct($name = '', $tablePrefix = '', $connection = '')
    {
        ...
        if (is_null($tablePrefix)) {
            ...
            $this->tablePrefix = SystemFun::Config('DB_PREFIX');
        }
    ...
    protected function _checkTableInfo()
    {
        ...
        if (empty($this->fields)) {
            // 如果数据表字段没有定义则自动获取
            if (SystemFun::Config('DB_FIELDS_CACHE')) {
    ...
    public function __call($method, $args)
    {
        if (in_array(strtolower($method), $this->methods, true)) {
            ...
            // 根据某个字段获取记录
            $field         = SystemFun::parseName(substr($method, 5));
            $where[$field] = $args[0];
            return $this->where($where)->find();
        } elseif (strtolower(substr($method, 0, 10)) == 'getfieldby') {
            // 根据某个字段获取记录的某个值
            $name         = SystemFun::parseName(substr($method, 10));
            $where[$name] = $args[0];
    ...
    {
        // 检查字段映射
        if (!empty($this->_map) && SystemFun::Config('READ_DATA_MAP')) {
    ...
    public function create($data = '', $type = '')
    {
        ...
        if (isset($fields)) {
            ...
            // 判断令牌验证字段
            if (SystemFun::Config('TOKEN_ON')) {
                $fields[] = SystemFun::Config('TOKEN_NAME', null, '__hash__');
            }
    ...
    public function autoCheckToken($data)
    {
        ...
        if (SystemFun::Config('TOKEN_ON')) {
            $name = SystemFun::Config('TOKEN_NAME', null, '__hash__');
            ...
            // 开启TOKEN重置
            if (SystemFun::Config('TOKEN_RESET')) {
    ...
    public function db($linkNum = '', $config = '', $force = false)
    {
        ...
        if (!isset($this->_db[$linkNum]) || $force) {
            ...
            if (!empty($config) && is_string($config) && false === strpos($config, '/')) {
                // 支持读取配置参数
                $config = SystemFun::Config($config);
            }
    ...
    {
        if (empty($this->name)) {
            $name = substr(get_class($this), 0, -strlen(SystemFun::Config('DEFAULT_M_LAYER')));
            if ($pos = strrpos($name, '\\')) {
                //有命名空间
    ...
    public function getTableName()
    {
        if (empty($this->trueTableName)) {
            ...
            if (!empty($this->tableName)) {
                ...
                $tableName .= SystemFun::parseName($this->name);
            }
